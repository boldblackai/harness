...
**Home-directory guard:** After arg parsing, if the resolved cwd equals the resolved home directory (via `fs.realpathSync`, so symlinked homes still match) and `--mount-entire-home` is not passed, harness exits with an error. Mounting all of `$HOME` into the container would expose dotfiles, credentials, and SSH keys to the agent and produce a noisy workspace. The guard only applies when the cwd is what gets mounted, so `--file` mode (single-file mount) is unaffected. `--mount-entire-home` opts back in for the rare case where mounting `$HOME` is intended.
