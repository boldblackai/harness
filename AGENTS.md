...
**Persistence:** Interactive runs (no `-p`, no piped stdin, no `--ephemeral`) bind-mount `XDG_STATE_HOME/harness/<normalized-cwd>/<agent>/` (defaults to `~/.local/state/harness/…`) into the container. The `<normalized-cwd>` is the working directory relative to `$HOME` with slashes converted to dashes (paths outside `$HOME` keep their full path). Each adapter declares its own mount points via `persistMounts()`. One-shot runs are implicitly ephemeral. Cosign cache uses `XDG_CACHE_HOME` (defaults to `~/.cache/harness/cosign-verified.json`).
